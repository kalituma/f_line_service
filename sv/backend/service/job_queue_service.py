...
class JobQueueService:
    ...
    def add_job(self, work_id: int, frfr_id: str, analysis_id: str, video_url: str, status: WorkStatus = WorkStatus.PENDING) -> Optional[int]:
        ...
        """
        try:
            job_id = self.job_queue.add_job(work_id, frfr_id, analysis_id, video_url, status)
            if job_id:
    ...
    def add_job_with_tasks(self, work_id: int, frfr_id: str, analysis_id: str, video_url: str, task_names: List[str]) -> Optional[int]:
        """
        ...
        try:
            # 작업 추가
            job_id = self.job_queue.add_job(work_id, frfr_id, analysis_id, video_url, WorkStatus.PENDING)
            if not job_id:
    ...
    def get_job_tasks(self, job_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Tasks retrieved for job_id={job_id}, count={len(result)}")
            return result
        except Exception as e:
            logger.error(f"Error getting job tasks: {str(e)}")
            raise
    
    def update_task_status(self, task_id: int, status: WorkStatus) -> bool:
        """
        태스크의 상태를 업데이트합니다.
    ...
    def insert_tasks_from_task_objects(self, job_id: int, tasks: List['TaskBase'], job_work_dir: str) -> Optional[List[int]]:
        """
        Task 객체들로부터 작업 정보를 추출하여 데이터베이스에 저장합니다.

        Args:
            job_id: 작업 ID
            tasks: Task 객체 리스트
            job_work_dir: Job 작업 디렉토리 경로

        Returns:
            생성된 task_id 리스트 (실패 시 None)
        """
        try:
            # 각 task의 순번과 이름을 추출하여 (seq, task_name, workspace) 튜플 생성
            tasks_with_workspace = []
            for seq, task in enumerate(tasks, start=1):
                task_name = task.task_name
                # job_work_dir에 task 이름을 join한 경로를 workspace로 지정
                workspace = os.path.join(job_work_dir, task_name)
                tasks_with_workspace.append((seq, task_name, workspace))

            # 데이터베이스에 저장 (task_id 리스트 반환)
            task_ids = self.task_queue.insert_tasks(job_id, tasks_with_workspace)

            if task_ids:
                logger.info(f"Tasks saved successfully: job_id={job_id}, count={len(tasks_with_workspace)}, task_ids={task_ids}")
                for (seq, task_name, workspace), task_id in zip(tasks_with_workspace, task_ids):
                    logger.debug(f"  - task_id={task_id}, seq={seq}, task_name={task_name}, workspace={workspace}")
            else:
                logger.error(f"Failed to save tasks: job_id={job_id}")

            return task_ids

        except Exception as e:
            logger.error(f"Error inserting tasks from task objects: {str(e)}", exc_info=True)
            return None